package conf

import (
	"fmt"
	"strings"

	"code.huawink.com/beiwanglu/mq-server/model"
	"gopkg.in/ini.v1"
)

//mysql,mq配置信息
var (
	Db               string
	DbHost           string
	DbPort           string
	DbUser           string
	DbPassWord       string
	DbName           string
	RabbitMQ         string
	RabbitMQUser     string
	RabbitMQPassWord string
	RabbitMQHost     string
	RabbitMQPort     string
)

//数据库初始化，进行数据池连接
//导入model的连接时的初始化
func Init() {
	//读取ini文件的数据库配置信息
	file, err := ini.Load("./conf/config.ini")
	if err != nil {
		fmt.Println("配置文件读取错误,请检查文件路径:", err)
	}
	LoadMySQL(file)
	pathMySQL := strings.Join([]string{DbUser, ":", DbPassWord, "@tcp(", DbHost, ":", DbPort, ")/", DbName, "?charset=utf8&parseTime=true"}, "")
	model.Database(pathMySQL)

	// 连接RabbitMQ
	LoadRabbitMQ(file)
	pathRabbitMQ := strings.Join([]string{RabbitMQ, "://", DbPassWord, "@tcp(", DbHost, ":", DbPort, ")/", DbName, "?charset=utf8&parseTime=true"}, "")
	model.RabbitMQ(pathRabbitMQ)
}

//加载mysql配置信息
func LoadMySQL(file *ini.File) {
	//file.Section("mysql").Key("Db").String()是在读取config.ini 的信息
	Db = file.Section("mysql").Key("Db").String()
	DbHost = file.Section("mysql").Key("DbHost").String()
	DbPort = file.Section("mysql").Key("DbPort").String()
	DbUser = file.Section("mysql").Key("DbUser").String()
	DbPassWord = file.Section("mysql").Key("DbPassWord").String()
	DbName = file.Section("mysql").Key("DbName").String()
}

//加载RabbitMQ配置信息
func LoadRabbitMQ(file *ini.File) {
	RabbitMQ = file.Section("rabbitmq").Key("RabbitMQ").String()
	RabbitMQUser = file.Section("rabbitmq").Key("RabbitMQUser").String()
	RabbitMQPassWord = file.Section("rabbitmq").Key("RabbitMQPassWord").String()
	RabbitMQHost = file.Section("rabbitmq").Key("RabbitMQHost").String()
	RabbitMQPort = file.Section("rabbitmq").Key("RabbitMQPort").String()
}
